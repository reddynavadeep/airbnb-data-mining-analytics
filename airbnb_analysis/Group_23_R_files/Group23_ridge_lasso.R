...
#start to train ridge and lasso model
set.seed(1)
library(glmnet)
...
#formula of ridge and lasso model
...
# Start to train Ridge and Lasso Madel
...
train_x<-train_log%>%select(-perfect_rating_score)
train_y<-train_log$perfect_rating_score


valid_x<-valid_log%>%select(-perfect_rating_score)
vaild_y<-valid_log$perfect_rating_score
...
unreg <- glmnet(train_x, train_y, family = "binomial", alpha = 0, lambda = 0)
preds <- predict(unreg, newx = as.matrix(train_x), type = "response")
...
#storage vector
TPRs <- rep(0, length(grid))

for(i in c(1:length(grid))){
  lam = grid[i] #current value of lambda
  
  #train a ridge model with lambda = lam
  glmout <- glmnet(train_x, train_y, family = "binomial", alpha = 1, lambda = lam)
  
  #make predictions as usual
  preds <- predict(glmout, newx = as.matrix(valid_x), type = "response")
  preds_full<-prediction(preds, vaild_y)
...
plot(log10(grid), TPRs)

# get best-performing lambda
best_validation_index <- which.max(TPRs)
...
unreg <- glmnet(train_x, train_y, family = "binomial", alpha = 0, lambda = best_lambda)
preds <- predict(unreg, newx = as.matrix(valid_x), type = "response")


pred_full <- prediction(preds,vaild_y)
...
#### TPR and FPR for train dataset
preds <- predict(unreg, newx = as.matrix(train_x), type = "response")
pred_full <- prediction(preds,train_y)
