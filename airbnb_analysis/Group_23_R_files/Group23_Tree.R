...
train_perfect$perfect_rating_score<-ifelse(is.na(train_perfect$perfect_rating_score),"NO",train_perfect$perfect_rating_score)
...
#start to train tree model
set.seed(1)
## Partition 30% of the data as sample
sample_test = sample(nrow(train_perfect), 0.10*nrow(train_perfect))
train_perfect<-train_perfect[sample_test,]
...
#formula of tree model
formula_tree<-perfect_rating_score ~ .
...
tr_pred <- function(train_data, valid_data, model_formula){
  trained_model <- lm(data = train_data, model_formula) 
...
library(tree)
mycontrol = tree.control(nobs = nrow(train_log), mincut = 1, minsize = 2, mindev = 0)
full_tree <- tree(formula_tree,
                  data = train_log, 
                  control = mycontrol)
...
# default_tree=tree(formula_tree,train_log)
summary(full_tree)

pruned_tree_100=prune.tree(full_tree, best = 100)

tree_preds <- predict(pruned_tree_100, newdata=valid_log)

tree_probs=tree_preds[,2]
tree_probs

pred_full <- prediction(tree_probs,valid_log$perfect_rating_score)
...
tree_preds <- predict(pruned_tree_100, newdata=train_log)

tree_probs=tree_preds[,2]
tree_probs

pred_full <- prediction(tree_probs,train_log$perfect_rating_score)
roc_full <- performance(pred_full, "tpr", "fpr")
...
#Build up fitting curve for tree
tree_predict_classify <- function(predicting_dataset, tree_name, cutoff){
  ...
  acc <- accuracy(predicting_dataset$perfect_rating_score, classifications)
  return(acc)
}
...
for (i in c(1:num_sizes)){
  ...
  tree_preds <- predict(inner_tree , newdata=valid_log)
  tree_probs=tree_preds[,2]
  tree_probs
  ...
  # inner_acc <- accuracy(classifications, vaild_y)
  TPRs[i] <- TPR[FPR_index]
}

#Plotting the values
plot(tree_sizes, TPRs)
