...
TPR[FPR_index]
cutoff[FPR_index]
################Evaluation of model using 10 fold cross validation##############

#K fold validation
#converting target variable into binary 
train_perfect <- train_perfect%>% mutate(perfect_rating_score = ifelse(perfect_rating_score=="YES",1,0))
#randomly shuffle the data
train_perfect_shuffle <- train_perfect[sample(nrow(train_perfect)),]
#view(train_perfect_shuffle)
#define k = the number of folds
k <- 10
#separate data into k equally-sized folds
#cut() will assign "fold numbers" to each instance
folds <- cut(seq(1,nrow(train_perfect_shuffle)),breaks=k,labels=FALSE)
# Make a vector of zeros to store your accuracy in each fold using the rep function
log1_folds = rep(0, k)
log2_folds = rep(0, k)
log3_folds = rep(0, k)
...
for(i in 1:k){
  #Segmenting data by fold using the which() function 
  valid_inds <- which(folds==i,arr.ind=TRUE)
  valid_fold <- train_perfect_shuffle[valid_inds, ]
  train_fold <- train_perfect_shuffle[-valid_inds, ]
  #Training the model
  model_kfold <- glm(formula_log, data = train_fold, family = "binomial") 
  #Making predictions
  predictions_kfold <- predict(model_kfold, newdata = valid_fold, type = "response")
  #Using the cutoff from model generated above 
  classifications_kfold <-  ifelse(predictions_kfold > cutoff[FPR_index], 1, 0)
  valid_fold_y <- as.factor(valid_fold$perfect_rating_score)
  CM <- confusionMatrix(data = as.factor(classifications_kfold), 
                        reference = valid_fold_y,
                        positive = "1")
  TP <- CM$table[2,2]
  TN <- CM$table[1,1]
  FP <- CM$table[2,1]
  ...
  log1_folds[i] <- TP/(TP+FN)
  log2_folds[i] <- as.numeric(CM$overall["Accuracy"])
  log3_folds[i] <- FP/(FP+TN)
}
#Storing accuracy, TPR and fpr values 
accuracy <- log2_folds
TPR_kfold <- log1_folds
FPR_kfold <- log3_folds
#Plotting TPR and FPR values for all folds
data_matrix <- rbind(TPR_kfold, FPR_kfold)
bar_names <- seq_along(TPR_kfold)
barplot(data_matrix, beside = TRUE, col = c("green", "orange"), xlab = "Index", ylab = "Value", main = "TPR and FPR",
        legend.text = c("TPR", "FPR"), args.legend = list(x = "topright"))