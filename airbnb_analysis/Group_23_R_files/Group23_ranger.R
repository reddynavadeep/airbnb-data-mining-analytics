...
airbnb_train <- perfect_data[1:dim(train_x)[1], ]
airbnb_test <- perfect_data[(dim(train_x)[1] + 1):(dim(train_x)[1] + dim(test_x)[1]), ]
...
# train_rang_x <- training x data.......valid_rang_x <- validation x data
# train_rang_y <- training dependent data.......valid_rang_y <- validation dependent variables
train_rang_x <- sampled_airbnb_train[-valid_instn, ]
train_rang_y <- sampled_train_y$perfect_rating_score[-valid_instn]
valid_rang_x <- sampled_airbnb_train[valid_instn, ]
valid_rang_y <- sampled_train_y$perfect_rating_score[valid_instn]


# Making the list of hyperparameters that I want to try
mtry_list = c(15, 20, 22, 25, 28, 30)
num_trees_list = c(300, 400, 500, 600, 700)
max_req_TPR = 0
best_mtry = mtry_list[1]
best_num_trees = num_trees_list[1]

for(i in c(1:length(mtry_list))){
  print(i)
  for(j in c(1:length(num_trees_list))){
    # Training the Random Forest model
    rf.model <- ranger(x = mtry_list[i], y = num_trees_list[j],
                       mtry=22, num.trees=500,
                       importance="impurity",
                       probability = TRUE)
    # Predicting dependent variable
    preds_bst <- predict(rf.model, data=valid_rang_x)$predictions[,2]
    # Making a prediction object to caluclate TPR, and FPR
    pred_full <- prediction(preds_bst, valid_rang_y)
    # Making a ROC object
    ...
    if(req_TPR > max_req_TPR){
      max_req_TPR = req_TPR
      best_mtry = mtry_list[i]
      best_num_trees = num_trees_list[j]
    }
  }
}
...
# train_rang_x <- training x data.......valid_rang_x <- validation x data
# train_rang_y <- training dependent data.......valid_rang_y <- validation dependent variables
train_rang_x <- airbnb_train[-valid_instn, ]
train_rang_y <- train_y$perfect_rating_score[-valid_instn]
valid_rang_x <- airbnb_train[valid_instn, ]
valid_rang_y <- train_y$perfect_rating_score[valid_instn]


set.seed(12345)
# Training the ranger model with tuned hyperparameters
rf.model <- ranger(x = train_rang_x, y = train_rang_y,
                   mtry=22, num.trees=500,
                   importance="impurity",
                   probability = TRUE)
...
# Predicting dependent variable
preds_bst <- predict(rf.model, data=train_rang_x)$predictions[,2]
# Making a prediction object to caluclate TPR, and FPR
pred_full <- prediction(preds_bst, train_rang_y)
# Making a ROC object
roc_full <- performance(pred_full, "tpr", "fpr")
# Storing TPR, FPR, and cut-off values
TPR<-roc_full@y.values
...
num_trees_list <- c(300, 350, 400, 450, 500, 550, 600, 650, 700, 750)
# Make a vector to store all the TPR values
TPR_list = rep(0,length(num_trees_list))
...
for(index in c(1:length(TPR_list))){
  ...
  # Training the ranger model with tuned hyperparameters
  rf.model <- ranger(x = train_rang_x, y = train_rang_y,
                     mtry=22, num.trees=num_trees_list[index],
                     importance="impurity",
                     probability = TRUE)
...
plot(num_trees_list, TPR_list, type="o", col="blue", ylim=c(min(TPR_list),max(TPR_list)))
